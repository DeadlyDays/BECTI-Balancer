        ...
        public void populateFromCSV(String path, String type)
        {
            ...
            //Populate Units
            {
                ...
                //iterate through content
                {
                    ...
                    newItem.addField(new Field("_o", "", "", "Ordinance Type", "\""), false);
                    newItem.addField(new Field("_u", "", "", "Upgrade Level"), false);
                    newItem.addField(new Field("_p", "", "", "Price"), false);
                    newItem.addField(new Field("_t", "", "", "Rearm time per round "), false);
                    newItem.FieldList[0].DisplayName = "ClassName";
                    newItem.FieldList[0].Tags = "\"";
                    addItem(newItem, type);
                    //Add each line to item
            ...
            //Populate Units
            {
                ...
                if ((_i.Count == _o.Count) && (_i.Count == _u.Count) && (_i.Count == _p.Count) && (_i.Count == _t.Count))
                    for (int i = 0; i < _i.Count; i++)
                    {
                        ...
                        newItem.addField(new Field("_i", "", _i[i], "Classname", "\""), false);
                        newItem.addField(new Field("_o", "", _o[i], "Ordinance Type", "\""), false);
                        newItem.addField(new Field("_u", "", _u[i], "Upgrade Level"), false);
                        newItem.addField(new Field("_p", "", _p[i], "Price"), false);
                        newItem.addField(new Field("_t", "", _t[i], "Rearm time per round "), false);
                        addItem(newItem, type);
                    }
